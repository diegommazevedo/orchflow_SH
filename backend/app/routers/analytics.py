...
def get_roi(
    ...
    """Métricas consolidadas de ROI por usuário. Requer autenticação."""
    ...
    all_tasks: list[Task] = db.query(Task).filter(
        Task.deleted_at.is_(None),
        Task.project_id.in_(_ws_project_ids),
    ).all()

    tasks_completed_total = sum(1 for t in all_tasks if task_is_done(db, t))
    ...
    # ── 6. Projects ───────────────────────────────────────────────────────────
    projects = db.query(Project).filter(Project.workspace_id == current_workspace.id).all()
    projects_data = []
    ...
    for proj in projects:
        ...
        tasks_done  = sum(1 for t in ptasks if task_is_done(db, t))
        ...
        for t in ptasks:
            ...
            if q in q_by_quad:
                q_by_quad[q] += 1
            if t.due_date_iso and t.due_date_iso < today_iso and not task_is_done(db, t):
                overdue += 1
