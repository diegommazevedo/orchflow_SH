...
def start_session(
    ...
    if task_id:
        t = db.query(Task).join(Project, Project.id == Task.project_id).filter(
            Task.id == task_id, Project.workspace_id == current_workspace.id
        ).first()
        if not t:
            raise HTTPException(status_code=404, detail="Task não encontrada no workspace")
...
def end_session(
    ...
    if session.user_id != str(current_user.id):
        raise HTTPException(status_code=403, detail="Sessão não pertence ao usuário atual")
    if not session:
    ...
    if session.task_id:
        task = (
            ...
            .filter(Task.id == session.task_id, Project.workspace_id == current_workspace.id)
            .first()
        )
        if task:
            task.time_spent_minutes = (task.time_spent_minutes or 0) + duration_min
            task_completed = task_is_done(db, task)
...
def get_active_session(
    ...
    session = (
        db.query(FocusSession)
        .outerjoin(Task, Task.id == FocusSession.task_id)
        .outerjoin(Project, Project.id == Task.project_id)
        .filter(
            FocusSession.user_id == user_id,
            FocusSession.ended_at == None,  # noqa: E711
            or_(FocusSession.task_id.is_(None), Project.workspace_id == current_workspace.id),
        )
        .first()
    )
    if not session:
