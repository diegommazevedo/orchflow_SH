...
from app.models.task import EisenhowerQuadrant, Task
from app.models.user import User
from app.models.workspace import Workspace
from app.kanban_helpers import task_is_done, task_status_slug
...
def roi_summary(
    ...
    """
    Agrega métricas ROI.
    project_id = None → visão global (todos os projetos do workspace).
    """
    ws_project_ids = [
        r[0] for r in db.query(Project.id).filter(Project.workspace_id == current_workspace.id).all()
    ]
    q = db.query(Task).filter(
        Task.deleted_at.is_(None),
        Task.project_id.in_(ws_project_ids),
    )
    if project_id:
    ...
    # ── status counts (slugs dinâmicos V2)
    status_counts: dict[str, int] = {}
    for t in tasks:
        s = task_status_slug(t)
    ...
    done_this_week = sum(
        1 for t in tasks
        if task_is_done(db, t) and t.created_at and t.created_at.date() >= week_start
    )
    done_this_month = sum(
        1 for t in tasks
        if task_is_done(db, t) and t.created_at and t.created_at.date() >= month_start
    )
...
def roi_timeline(
    ...
    for t in tasks:
        ...
        if task_is_done(db, t) and t.created_at:
            d = t.created_at.date().isoformat()
            if d in day_map:
