...
def _get_project(project_id: str, db: Session, workspace: "Workspace | None" = None) -> Project:
    pid = _parse_project_id(project_id)
    q = db.query(Project).filter(Project.id == pid)
    if workspace is not None:
        q = q.filter(Project.workspace_id == workspace.id)
    p = q.first()
    if not p:
...
def export_backlog(
    ...
    Somente leitura — zero escrita no banco.
    """
    project = _get_project(project_id, db, current_workspace)
