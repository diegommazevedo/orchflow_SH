...
def execute_intent(
    ...
    # Valida que project_id pertence ao workspace antes de qualquer ação
    if project_id and workspace_id:
        ...
        if not project_check:
            return {"success": False, "message": "Projeto não encontrado no workspace."}
    ...
    if action == "update_task_status":
        ...
        raw_st = (params.status or "in_progress").strip()[:64]
        task.status = raw_st if raw_st else "in_progress"
        db.commit()

        labels = {"backlog": "Backlog", "in_progress": "Em andamento", "done": "Concluído", "blocked": "Impedimento"}
        lbl = labels.get(task.status, task.status)
        return {
            "success": True,
            "message": f'✓ "{task.title}" → {lbl}',
        }
    ...
    if action == "delete_task":
        ...
        task.deleted_by = user_id or "system"
        db.add(ActivityLog(
            entity_type="task",
            entity_id=task.id,
    ...
    if action == "list_projects":
        q = db.query(Project)
        if workspace_id:
            q = q.filter(Project.workspace_id == uuid.UUID(workspace_id))
        projects = q.all()
        if not projects:
