...
export function relativeTime(isoString: string): string {
  ...
  const dThen = new Date(isoString)
  const dNow  = new Date()
  const daysBetween = Math.round((startOfDay(dNow) - startOfDay(dThen)) / 86_400_000)

  if (daysBetween === 0) return 'hoje'
  if (daysBetween === 1) return 'ontem'
  if (daysBetween < 7)   return `${daysBetween} dias atrás`

  return dThen.toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' })
}
...
export const ACTION_LABELS: Record<ActivityAction, (meta: Record<string, unknown>) => string> = {
  ...
  template_applied:   (m) => `template aplicado: ${m.template_name ?? m.template_slug ?? '?'}`,
  schema_agent_applied: (m) => {
    const accepted = Array.isArray(m.accepted) ? m.accepted.length : 0
    const rejected = Array.isArray(m.rejected) ? m.rejected.length : 0
    return `schema agent aplicado (${accepted} aceitas, ${rejected} rejeitadas)`
  },
  field_created:      (m) => `campo criado: ${m.label ?? m.name ?? '?'}`,
  field_deleted:      (m) => `campo removido: ${m.name ?? '?'}`,
}
