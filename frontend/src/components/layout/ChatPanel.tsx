...
export function ChatPanel({
  ...
  useEffect(() => {
    const saved = localStorage.getItem('orchflow:chat:minimized')
    setIsMinimized(saved === 'true')
  }, [])

  useEffect(() => {
    localStorage.setItem('orchflow:chat:minimized', isMinimized ? 'true' : 'false')
    if (!isMinimized) setHasUnreadWhileMinimized(false)
  }, [isMinimized])
  ...
  useEffect(() => {
    ...
    if (isMinimized && lastMsg?.role === 'agent') {
  ...
  return (
    <div className={`chat-panel${isMinimized ? ' chat-panel-minimized' : ''}`}>
      ...
      <div className="chat-header">
        ...
        {hasUnreadWhileMinimized && <span className="chat-unread-dot" title="Nova mensagem">●</span>}
