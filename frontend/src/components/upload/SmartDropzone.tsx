...
export function SmartDropzone({ onContractResult, onSheetResult, onAudioTranscript, projectId }: Props) {
  ...
  const processFile = useCallback(async (file: File) => {
    ...
    try {
      // ── 1. Detectar tipo ─────────────────────────────────────────────────
      // Sobrescreve o Content-Type padrão da instância (application/json)
      // para multipart/form-data — necessário para UploadFile no FastAPI.
      const MULTIPART = { headers: { 'Content-Type': 'multipart/form-data' } }

      const fd1 = new FormData()
      fd1.append('file', file)
      const { data: detect } = await api.post<DetectResult>('/upload/detect', fd1, MULTIPART)
      ...
      switch (detect.file_type) {
        case 'contract_pdf': {
          ...
          const { data } = await api.post<ContractParseResult>('/upload/contract', fd2, MULTIPART)
          onContractResult(data)
          break
        }
        case 'sheet': {
          fd2.append('file', file)
          const { data } = await api.post<SheetParseResult>('/upload/sheet', fd2, MULTIPART)
          onSheetResult(data)
          break
        }
        case 'audio': {
          fd2.append('audio', file)
          const { data } = await api.post<{ text: string }>('/voice/transcribe', fd2, MULTIPART)
